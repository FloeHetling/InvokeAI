...
_CHROMA_T5_TARGET_MODEL_VRAM_BYTES = 8 * 2**30
...
def _get_chroma_t5_working_mem_bytes(device: torch.device) -> int | None:
    """Reserve VRAM so Chroma T5 stays partially resident instead of filling the device."""
    if device.type != "cuda":
        return None

    total_vram_bytes = torch.cuda.get_device_properties(device).total_memory
    return max(0, total_vram_bytes - _CHROMA_T5_TARGET_MODEL_VRAM_BYTES)
...
class ChromaTextEncoderInvocation(BaseInvocation):
    ...
    def invoke(self, context: InvocationContext) -> FluxConditioningOutput:
        ...
        t5_working_mem_bytes = _get_chroma_t5_working_mem_bytes(device)

        with (
            text_encoder_info.model_on_device(working_mem_bytes=t5_working_mem_bytes) as (cached_weights, text_encoder),
