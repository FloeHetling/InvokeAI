...
class ChromaDenoiseInvocation(FluxDenoiseInvocation):
    ...
    scheduler: CHROMA_SCHEDULER_NAME_VALUES = InputField(
        default="euler",
    ...
    def _run_diffusion(self, context: InvocationContext) -> torch.Tensor:
        ...
        positive = self._load_chroma_conditioning(context, self.positive_text_conditioning, inference_dtype, device)
        negative = (
            self._load_chroma_conditioning(context, self.negative_text_conditioning, inference_dtype, device)
        ...
        if (
            self.scheduler not in {"euler", "euler_cfg_pp_beta"}
            and self.scheduler in FLUX_SCHEDULER_MAP
        ):
